...
<script src="<?php echo e(asset('js/extensions/mobile/bootstrap-table-mobile.js')); ?>"></script>

<?php if(!isset($simple_view)): ?>
<script src="<?php echo e(asset('js/extensions/export/bootstrap-table-export.js?v=1')); ?>"></script>
<script src="<?php echo e(asset('js/extensions/cookie/bootstrap-table-cookie.js?v=1')); ?>"></script>
<script src="<?php echo e(asset('js/extensions/export/tableExport.js')); ?>"></script>
<script src="<?php echo e(asset('js/FileSaver.min.js')); ?>"></script>
<script src="<?php echo e(asset('js/jspdf.min.js')); ?>"></script>
<script src="<?php echo e(asset('js/jspdf.plugin.autotable.js')); ?>"></script>
<script src="<?php echo e(asset('js/extensions/export/jquery.base64.js')); ?>"></script>
<script src="<?php echo e(asset('js/extensions/toolbar/bootstrap-table-toolbar.js')); ?>"></script>
<script src="<?php echo e(asset('js/extensions/sticky-header/bootstrap-table-sticky-header.js')); ?>"></script>
<?php endif; ?>
...
<script nonce="<?php echo e(csrf_token()); ?>">
    ...
    function buildTable($el) {
        ...
        sidePagination: '<?php echo e((isset($clientSearch)) ? 'client' : 'server'); ?>',
        sortable: true,
    ...
    function genericColumnObjLinkFormatter(destination) {
        return function (value,row) {
            if ((value) && (value.status_meta)) {
                ...
                return '<nobr><a href="<?php echo e(url('/')); ?>/' + destination + '/' + value.id + '" data-tooltip="true" title="'+ value.status_meta + '"> <i class="fa ' + icon_style + ' text-' + text_color + '"></i> ' + value.name + '</a></nobr>';
            } else if ((value) && (value.name)) {
                return '<nobr><a href="<?php echo e(url('/')); ?>/' + destination + '/' + value.id + '"> ' + value.name + '</a></span>';
            }
        };
    }
    ...
    function genericActionsFormatter(destination) {
        return function (value,row) {
            ...
            if ((row.available_actions) && (row.available_actions.clone === true)) {
                actions += '<a href="<?php echo e(url('/')); ?>/' + dest + '/' + row.id + '/clone" class="btn btn-sm btn-info" data-tooltip="true" title="Clone"><i class="fa fa-copy"></i></a>&nbsp;';
            }

            if ((row.available_actions) && (row.available_actions.update === true)) {
                actions += '<a href="<?php echo e(url('/')); ?>/' + dest + '/' + row.id + '/edit" class="btn btn-sm btn-warning" data-tooltip="true" title="Update"><i class="fa fa-pencil"></i></a>&nbsp;';
            }
            ...
            if ((row.available_actions) && (row.available_actions.restore === true)) {
                actions += '<a href="<?php echo e(url('/')); ?>/' + dest + '/' + row.id + '/restore" class="btn btn-sm btn-warning" data-tooltip="true" title="Restore"><i class="fa fa-retweet"></i></a>&nbsp;';
            }
    ...
    function polymorphicItemFormatter(value) {
        ...
        if ((value) && (value.type)) {
            ...
            return '<nobr><a href="<?php echo e(url('/')); ?>/' + item_destination +'/' + value.id + '" data-tooltip="true" title="' + value.type + '"><i class="fa ' + item_icon + ' text-blue"></i> ' + value.name + '</a></nobr>';
    ...
    function genericCheckinCheckoutFormatter(destination) {
        return function (value,row) {
            ...
            if ((row.available_actions.checkout == true) && (row.user_can_checkout == true) && (!row.assigned_to)) {
                // case for licenses
                if (row.next_seat) {
                    return '<a href="<?php echo e(url('/')); ?>/' + destination + '/' + row.next_seat + '/checkout" class="btn btn-sm bg-maroon" data-tooltip="true" title="Check this item out to a user"><?php echo e(trans('general.checkout')); ?></a>';
                } else {
                    return '<a href="<?php echo e(url('/')); ?>/' + destination + '/' + row.id + '/checkout" class="btn btn-sm bg-maroon" data-tooltip="true" title="Check this item out to a user"><?php echo e(trans('general.checkout')); ?></a>';
                }


            // The user is allowed to check items out, but the item is not deployable
            } else if (((row.user_can_checkout == false)) && (row.available_actions.checkout == true) && (!row.assigned_to)) {
                return '<div  data-tooltip="true" title="This item has a status label that is undeployable and cannot be checked out at this time."><a class="btn btn-sm bg-maroon disabled"><?php echo e(trans('general.checkout')); ?></a></div>';

            // The user is allowed to check items in
            } else if (row.available_actions.checkin == true)  {
                if (row.assigned_to) {
                    return '<nobr><a href="<?php echo e(url('/')); ?>/' + destination + '/' + row.id + '/checkin" class="btn btn-sm bg-purple" data-tooltip="true" title="Check this item in so it is available for re-imaging, re-issue, etc."><?php echo e(trans('general.checkin')); ?></a>';
                } else if (row.assigned_pivot_id) {
                    return '<nobr><a href="<?php echo e(url('/')); ?>/' + destination + '/' + row.assigned_pivot_id + '/checkin" class="btn btn-sm bg-purple" data-tooltip="true" title="Check this item in so it is available for re-imaging, re-issue, etc."><?php echo e(trans('general.checkin')); ?></a>';
                }
    ...
    function deployedLocationFormatter(row, value) {
        if ((row) && (row!=undefined)) {
            ...
            return '<a href="<?php echo e(url('/')); ?>/locations/' + value.rtd_location.id + '" data-tooltip="true" title="Default Location"> ' + value.rtd_location.name + '</a>';
        }
   ...
   function imageFormatter(value, row) {
        if (value) {
            return '<a href="' + value + '" data-toggle="lightbox" data-type="image"><img src="' + value + '" style="max-height: <?php echo e($snipeSettings->thumbnail_max_h); ?>px; width: auto;" class="img-responsive"></a>';
        }
    }
